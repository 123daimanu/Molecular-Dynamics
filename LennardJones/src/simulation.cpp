...
void simulation::setParams(int num, float l, float r, float ene, float m,
                           float timeinterval, int Totaltime) {
  number = num;
  t = Totaltime;      // in fs
...
void simulation::update() {
  ...
  for (int i = 0; i < number; i++) {
    this->systemAtoms[i].vel = this->systemAtoms[i].vel +
                               ((this->systemAtoms[i].force) / 2) * (dt / 2);
    this->systemAtoms[i].pos =
        this->systemAtoms[i].pos + (this->systemAtoms[i].vel) * dt;
